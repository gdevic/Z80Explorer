...
// Contains information about a pull-up transistor (depletion load). These are
// not part of the functional m_transvdefs set; they exist on the die but are
// encoded as the `has_pullup` flag on a net. Detected from the feature bitmap.
struct pullupdef
{
    net_t  net;                         // Signal net this transistor pulls up
    QRect  box;                         // Bounding rectangle on the die
    int    w;                           // Channel width, pixels (larger bbox dimension)
    int    l;                           // Channel length, pixels (smaller bbox dimension)
};

/*
 * ClassVisual contains functions to hold the visual chip data and to manipulate with a set of die images,
 * some loaded and some generated by this class code
 */
class ClassVisual: public QObject
{
    Q_OBJECT
...
public:
    ...
    void drawPullups(QPainter &painter, const QRect &viewport);
    int  getPullupCount() const { return m_pullups.size(); }
...
private:
    ...
    QVector<pullupdef> m_pullups;       // Array of pull-up (depletion-load) transistors
    int m_typicalPullupSide {};         // Typical pull-up minor dimension (px); zoom gate for drawPullups
    QString m_resourceDir;              // Resource directory, captured at loadChipResources()
