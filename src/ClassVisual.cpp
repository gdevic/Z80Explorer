...
bool ClassVisual::loadImages(QString dir)
{
    ...
    QtConcurrent::mapped(files, [dir](const QString &image) {
        QImage img;
        QString png_file = dir + "/chip/z80_" + image + ".png";
...
{
    QString fileName = dir + "/chip/layermap.bin";
 ...
 * and provided as file "chip/layermap.bin". That one contains all the nets.
 ******************************************************************************/
...
{
    QSettings settings;
    QString fileName = settings.value("ResourceDir").toString() + "/chip/layermap.bin";
 ...
 * in the loop). We append those by reading a resource file "user/latches.ini"
 ****************************************************************************************/
...
{
    QSettings settings;
    QString fileName = settings.value("ResourceDir").toString() + "/user/latches.ini";
    qInfo() << "Loading" << fileName;
    QFile file(fileName);
    if (file.open(QFile::ReadOnly | QFile::Text))
    {
...
bool ClassVisual::savePullups()
{
    ...
    QJsonObject root;
    root["pullups"] = arr;
    QFile f(m_resourceDir + "/user/pullups.json");
